...
public class RedisTool {
	...
	public RedisTool(Environment env, RedisTemplate<String, Object> redisTemplate,
	...
	RedisTemplate<String, ?> template(int t) {
		return t == 0 ? redisTemplate : stringRedisTemplate;
	}
	 ...
	 * 在key前面附加前缀
	 * @param key
	 * @param mode NONE-不带任何应用前缀 APP-使用spring.application.name前缀
	 * APP_GROUP-使用application.group前缀，如果没有它则使用spring.application.name前缀
	 */
	public String wrapKey(String key, ModeDict mode) {
		// jdk17+
		/*
		 * return switch (mode) { case APP -> { if (springApplicationName != null &&
		 * !springApplicationName.isEmpty()) { yield springApplicationName + ":" + key; }
		 * yield key; } case APP_GROUP -> { if (applicationGroup != null &&
		 * !applicationGroup.isEmpty()) { yield applicationGroup + ":" + key; } yield
		 * wrapKey(key, ModeDict.APP); } default -> key; };
		 */
		// jdk8
		switch (mode) {
			case APP:
				if (springApplicationName != null && !springApplicationName.isEmpty()) {
					return springApplicationName + ":" + key;
				}
				return key;
			case APP_GROUP:
				if (applicationGroup != null && !applicationGroup.isEmpty()) {
					return applicationGroup + ":" + key;
				}
				return wrapKey(key, ModeDict.APP);
			case NONE:
			default:
				return key;
		}
	 ...
	 * @param key 键
	 * @param time 时间(秒)
	 * @param mode 0-不带任何应用前缀 1-使用spring.application.name前缀
	...
	public void del(ModeDict mode, String... key) {
		if (key != null && key.length > 0) {
			if (key.length == 1) {
				...
				redisTemplate.delete(Arrays.stream(key).map(e -> wrapKey(e, mode)).collect(toList()));
			}
		}
	}
	// ============================String=============================
	...
	public <T> T get(String key, ModeDict mode) {
		...
		return (T) operation.get(wrapKey(key, mode));
	}
	 ...
	 * @return 值
	 */
	public <T> T hget(String key, String item, ModeDict mode, int type) {
		HashOperations<String, String, T> operation = (template(type)).opsForHash();
	 ...
	 * @return 对应的多个键值
	 */
	public <T> Map<String, T> hmget(String key, ModeDict mode, int type) {
		HashOperations<String, String, T> operation = (template(type)).opsForHash();
	 ...
	 * @param map 对应多个键值
	 */
	public <T> void hmset(String key, Map<String, T> map, ModeDict mode, int type) {
		HashOperations<String, String, T> operation = (template(type)).opsForHash();
	 ...
	 * @param time 时间(秒)
	 */
	public <T> void hmset(String key, Map<String, T> map, ModeDict mode, long time, int type) {
		hmset(key, map, mode, type);
		if (time > 0) {
	 ...
	 * @param value 值
	 */
	public <T> void hset(String key, String item, T value, ModeDict mode, int type) {
		(template(type)).opsForHash().put(wrapKey(key, mode), item, value);
	}
	 ...
	 */
	public <T> void hset(String key, String item, T value, ModeDict mode, long time, int type) {
		(template(type)).opsForHash().put(wrapKey(key, mode), item, value);
		if (time > 0) {
			expire(key, mode, time);
		}
	...
	public <T> Set<T> sGet(String key, ModeDict mode) {
		...
		return (Set<T>) operation.members(wrapKey(key, mode));
	}
	...
	public Long setRemove(String key, ModeDict mode, Object... values) {
		return redisTemplate.opsForSet().remove(wrapKey(key, mode), values);
	}
	// ===============================list=================================
	...
	public <T> List<T> lGet(String key, long start, long end, ModeDict mode) {
		return (List<T>) redisTemplate.opsForList().range(wrapKey(key, mode), start, end);
	}
	...
	public <T> T lGetIndex(String key, long index, ModeDict mode) {
		...
		return (T) operation.index(wrapKey(key, mode), index);
	}
