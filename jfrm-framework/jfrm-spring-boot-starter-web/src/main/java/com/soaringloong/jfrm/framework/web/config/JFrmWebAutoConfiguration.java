...
public class JFrmWebAutoConfiguration implements WebMvcConfigurer {
	 ...
	 * <p>
	 * 前提是这些路径要先在 SpringSecurity 中设置为可匿名访问，例如在 com.ruoyi.framework.config.SecurityConfig
	 * 中编程式设置 permitAll 匿名访问 或 web.ignoring().antMatchers(xxx) 忽略拦截路径
	 * </p>
	 * @param registry
	 */
	@Override
	public void addResourceHandlers(ResourceHandlerRegistry registry) {
		...
		// upload
		Set<String> uploadFileDirs = new HashSet<>();
		uploadFileDirs.add("file:" + ossProperties.getResPath() + "/upload/");
		uploadFileDirs.add("file:" + RuoYiConfig.getProfile() + "/upload/");
		String[] locations = settingProperties.getStaticCustomLocations().split(",");
		uploadFileDirs.addAll(Arrays.asList(locations));
		registry.addResourceHandler("/upload/**").addResourceLocations(uploadFileDirs.toArray(new String[0]));
