...
@ConditionalOnProperty(prefix = "springdoc.api-docs", name = "enabled", havingValue = "true", matchIfMissing = true) // 设置为
																														// false
																														// 时，禁用
public class JFrmSwaggerAutoConfiguration {
	...
	private static Parameter buildTenantHeaderParameter() {
		return new Parameter().name(WebFrameworkUtils.HEADER_TENANT_ID) // header 名
			...
			.schema(new IntegerSchema()._default(1L).name(WebFrameworkUtils.HEADER_TENANT_ID).description("租户编号")); // 默认：使用租户编号为
																													// 1
	}
